...
void ServerUpdateSystem::updateEntity(const EntityBuffer::Entity* entity_buffer)
{
    ...
    for (auto idx = 0; idx < num_cts; idx++)
    {
        ...
        case EntityBuffer::Component_Animation:
        {
            ...
            // Unpack animation names from flatbuffer and store in Component
            animation->direction = data->direction(); // true for right-facing, false for left-facing
            auto anim_names = data->names();        // A pointer to a 'flatbuffers::Vector<>'
            auto anims_size = anim_names->size();
            for (int num_anim = 0; num_anim < anims_size; num_anim++)
            {
                std::string anim_name = anim_names->Get(num_anim)->str();
                animation->store.insert(anim_name);

                // Load animation if we haven't seen it before
                if (animation->animations.find(anim_name) == animation->animations.end())
                {
                    Animation new_animation;  // Build from YAML config
                    m_graphicManager->loadAnimationFromYaml(new_animation, anim_name, "../../assets/config/animation.yaml");
                    animation->animations[anim_name] = new_animation;
                    odsloga("loaded animation: " << anim_name << "\n");
                }
            }
